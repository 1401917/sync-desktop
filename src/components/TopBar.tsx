...
async function openTauriWindow(id: string) {
  if (!isTauriRuntime()) {
  ...
  try {
    ...
    const next = new mod.WebviewWindow(id, {
      url: "/",
      title: "Sync",
      width: 1280,
      height: 800,
...
export function TopBar({ payload }: TopBarProps) {
  ...
  async function minimize() {
    ...
    if (win) await win.minimize().catch(() => undefined);
  }

  async function toggleMaximize() {
    const win = await getAppWindow();
    if (win) await win.toggleMaximize().catch(() => undefined);
  }
  ...
  async function openNewWindow() {
    await openTauriWindow(`sync-${Date.now()}`);
  }
  ...
  const menus: Record<string, MenuEntry[]> = {
    File: [
      ...
      { label: "Close Window", shortcut: "Ctrl+W", onSelect: close }
    ],
    Edit: [
    ...
    Window: [
      { label: "Minimize", shortcut: "Ctrl+M", onSelect: minimize },
      { label: "Maximize / Restore", onSelect: toggleMaximize },
      { separator: true, label: "" },
      { label: "New Window", shortcut: "Ctrl+Shift+N", onSelect: openNewWindow }
    ],
    Help: [
  ...
  async function startDragging(event: PointerEvent<HTMLElement>) {
    ...
    const win = await getAppWindow();
    if (win) await win.startDragging().catch(() => undefined);
  }
    ...
    >
      <div data-tauri-drag-region className="flex min-w-0 flex-1 items-center gap-1.5">
        ...
        <div data-tauri-drag-region className="ml-1 flex items-center gap-2">
          ...
          <span data-tauri-drag-region className="text-[12px] font-medium text-[#eeeeee]">{payload.appName}</span>
        </div>
          ...
          {Object.keys(menus).map((label) => {
            ...
            return (
              <div key={label} className="relative">
                ...
                {isOpen ? (
                  <div data-no-drag="true" className="absolute left-0 top-[26px] z-50 min-w-[200px] rounded-md border border-[#2d2d2d] bg-[#1c1c1c] p-1 shadow-[0_8px_28px_rgba(0,0,0,0.45)]">
                    {menus[label].map((entry, index) =>
                      entry.separator ? (
                        ...
                        <button
                          ...
                          onClick={() => { setOpenMenu(null); entry.onSelect?.(); }}
                        ...
                        >
                          ...
                          {entry.shortcut ? <span className="text-[10.5px] text-[#7a7a7a]">{entry.shortcut}</span> : null}
                        </button>
                      )
                    )}
                  </div>
      ...
      <div className="flex min-w-0 flex-1 items-center justify-end gap-1">
        ...
        <button data-no-drag="true" className="window-control" aria-label="Minimize" title="Minimize" onClick={minimize}><Minus size={13} /></button>
        <button data-no-drag="true" className="window-control" aria-label="Maximize" title="Maximize" onClick={toggleMaximize}><Maximize2 size={12} /></button>
        <button data-no-drag="true" className="window-control close" aria-label="Close" title="Close" onClick={close}><X size={13} /></button>
      </div>
    </header>
  );
}
